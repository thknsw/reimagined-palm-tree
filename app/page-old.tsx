...
import Flashcards from "@/components/study-app/flashcards"
import { ExamSimulation } from "@/components/study-app/exam-simulation"
import { ExamResults } from "@/components/study-app/exam-results"
import { ReferenceCharts } from "@/components/study-app/reference-charts"
...
export default function AWSStudyApp() {
  ...
  const [examSimulationActive, setExamSimulationActive] = useState(false)
  const [examSimulationQuestions, setExamSimulationQuestions] = useState<Scenario[]>([])
  ...
  const [examResultsState, setExamResultsState] = useState<any>(null); // Declare exam_results variable
  ...
  const getSpacedDueScenarios = useCallback(() => {
    const now = Date.now()
    const scenarios = shuffledScenarios.length > 0 ? shuffledScenarios : SCENARIOS
  ...
  }, [spacedRepData, shuffledScenarios])
    ...
    const loadProgress = async () => {
      ...
      try {
        if (user) {
          ...
          if (error) {
            console.log('[v0] No progress found in database, starting fresh')
          } else if (data?.progress_data) {
            const progress = data.progress_data
            if (progress.domainProgress) setDomainProgress(progress.domainProgress)
            if (progress.bookmarked) setBookmarkedScenarios(progress.bookmarked)
            if (progress.spacedRepData) setSpacedRepData(progress.spacedRepData)
            if (progress.quizHistory) setQuizHistory(progress.quizHistory)
            if (progress.studyPlan) setStudyPlan(progress.studyPlan)
            if (progress.attemptedIds) setAttemptedScenarios(new Set(progress.attemptedIds))
            if (progress.dailyQuestion) setDailyQuestion(progress.dailyQuestion)
            if (progress.studySessions) setStudySessions(progress.studySessions)
            if (progress.adaptiveData) setAdaptiveData(progress.adaptiveData)
            if (progress.latestPreTestResult) setLatestPreTestResult(progress.latestPreTestResult)
            if (progress.confidenceData) setConfidenceData(progress.confidenceData)
            if (progress.unlockedBadges) setUnlockedBadges(progress.unlockedBadges)
            if (progress.sessionStats) {
              setSessionStats((prev) => ({
                ...prev,
                attempted: progress.sessionStats.attempted || 0,
                correct: progress.sessionStats.correct || 0,
                incorrect: progress.sessionStats.incorrect || 0,
                bestStreak: progress.sessionStats.bestStreak || 0,
                currentStreak: progress.sessionStats.currentStreak || 0,
              }))
            }
          }
        } else {
  ...
  useEffect(() => {
    ...
    const dataToSave = {
      ...
      lastSaved: Date.now(),
      examResults: examResultsState, // Include exam_results in saved data
    }
  ...
  const handleExportProgress = () => {
    const data = {
      ...
      examResults: examResultsState, // Include exam_results in exported data
    }
  ...
  const handleResetProgress = async () => {
    if (confirm("Are you sure you want to reset all progress? This cannot be undone.")) {
      // Reset in database if logged in
      if (user) {
        const supabase = createClient()
  ...
  if (examResultsState) {
    return (
      <ExamResults
        results={examResultsState}
        onReviewAnswers={() => {
          setExamResultsState(null)
          // Could implement review mode here
        }}
        onRetakeExam={() => {
          setExamResultsState(null)
          setExamSimulationActive(true)
          const allQuestions = shuffledScenarios.length > 0 ? shuffledScenarios : SCENARIOS
          const selected = shuffleArray([...allQuestions]).slice(0, 65)
          setExamSimulationQuestions(selected)
        }}
        onBackToDashboard={() => {
          setExamResultsState(null)
        }}
  ...
  if (examSimulationActive) {
    return (
      <ExamSimulation
        questions={examSimulationQuestions}
        onComplete={(results) => {
          setExamSimulationActive(false)
          setExamResultsState(results)
        }}
        onExit={() => {
          setExamSimulationActive(false)
          setExamSimulationQuestions([])
        }}
      />
    )
  }
  ...
  if (showReferenceCharts) {
    return <ReferenceCharts onBack={() => setShowReferenceCharts(false)} />
  }
